...
<script>
//添加车辆类型
var addUrl=path+"vehicleModel/add";
//编辑车辆类型
var editUrl=path+"vehicleModel/edit";
//删除车辆类型
var deleteUrl=path+"vehicleModel/delete";

var dialogWidth=700,dialogHeight=350;

//查询
function filter() {
	EasyUI.grid.search("dg","vehicleModelSearchForm");
}
...
var add = function() {
	var wid = Dialog.open("添加", dialogWidth, dialogHeight,addUrl, [
		...
		EasyUI.form.submit("vehicleModelForm",addUrl, function(data){
			filter();
...
var edit = function(){
	...
	var wid = Dialog.open("编辑", dialogWidth, dialogHeight, editUrl+"?id="+r.ID, [ 
	    EasyUI.window.button("icon-save", "保存", function() {
		EasyUI.form.submit("vehicleModelForm", editUrl, function(data) {
			filter();
			Dialog.close(wid);
		})
	}), EasyUI.window.button("icon-cancel", "关闭", function() {
