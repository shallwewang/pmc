 ...
 */
@Repository
public class LineDaoImpl extends BaseDaoImpl implements ILineDao {
	...
	public <T> Map<String, Object> findPageInfo(Filter filter, Page page) throws Exception {
		return this.findPageInfo(filter,page,"line-list");
	}

	@Override
	public List<Map<String, Object>> combotree() {
		String sql =SQL.get("line-combotree");
	...
	public List<Map<String, Object>> tree(String workshop, String plant) {
		...
		if (!StringUtils.isEmpty(workshop))
			filter.set("workshop", workshop);
		if (!StringUtils.isEmpty(plant))
			filter.set("plant", plant);
		try {
			SQLQuery query = getSession().createSQLQuery(
					SQL.get(filter.getFilter(), "line-tree"));
